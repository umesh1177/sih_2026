...
export const TrainerDashboardView = ({ 
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 space-y-6 max-w-7xl mx-auto font-sans text-slate-800">
      ...
      {/* Toast Notification */}
      {toastMessage && (
        <div className="fixed bottom-6 right-6 z-50 flex items-center gap-3 px-4 py-3 rounded-[var(--radius)] bg-slate-900 text-white shadow-xl border border-slate-700 animate-in slide-in-from-bottom-5">
        ...
        <div className="flex items-center gap-2.5 shrink-0 flex-wrap sm:flex-nowrap">
          {onNavigatePerformance && (
            <button
              ...
              className="flex items-center gap-2 px-4 py-2.5 bg-slate-50 hover:bg-slate-100 text-slate-700 border border-slate-200 font-semibold rounded-[var(--radius)] text-xs transition-colors"
            >
          ...
          {onOpenQuestionBank && (
            <button
              ...
              className="flex items-center gap-2 px-4 py-2.5 bg-white border border-slate-300 hover:bg-slate-50 text-slate-700 font-semibold rounded-[var(--radius)] text-xs transition-colors"
            >
          ...
          <button
            ...
            className="flex items-center gap-2 px-4 py-2.5 bg-[#0B3475] hover:bg-[#08285C] text-white font-semibold rounded-[var(--radius)] text-xs shadow-xs transition-colors"
          >
      ...
      {integrityAlerts.length > 0 && (
        <div className="bg-red-50/70 border border-red-200 rounded-[var(--radius)] p-5 shadow-xs space-y-3 animate-in fade-in">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2.5">
              <div className="w-8 h-8 rounded-[var(--radius)] bg-red-600 text-white flex items-center justify-center font-medium">
                <ShieldAlert className="w-4 h-4" />
              </div>
              <div>
                <h3 className="text-xs font-semibold text-red-950">
            ...
            <span className="px-2 py-0.5 rounded text-[10px] font-medium bg-red-200 text-red-900 uppercase">
              Action Required
            </span>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3 pt-1">
            {integrityAlerts.map((alert, idx) => (
              <div
                key={idx}
                className="p-3.5 bg-white rounded-[var(--radius)] border border-red-200 shadow-xs space-y-2 flex flex-col justify-between"
              >
                <div className="space-y-1">
                  <div className="flex items-center justify-between">
                    <span className="font-medium text-slate-900 text-xs">{alert.traineeName}</span>
                ...
                <div className="pt-2 border-t border-slate-100 flex items-center justify-end">
                  <button
                    ...
                    className="inline-flex items-center gap-1 px-3 py-1.5 bg-amber-600 hover:bg-amber-700 text-white font-semibold rounded-[var(--radius)] text-xs transition-colors"
                  >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-white rounded-[var(--radius)] border border-slate-200/90 p-5 shadow-xs hover:border-slate-300 transition-colors flex items-center justify-between">
          <div className="space-y-1">
            ...
            <div className="text-2xl font-medium text-slate-900">{courses.length}</div>
        ...
        <div className="bg-white rounded-[var(--radius)] border border-slate-200/90 p-5 shadow-xs hover:border-slate-300 transition-colors flex items-center justify-between">
          <div className="space-y-1">
            ...
            <div className="text-2xl font-medium text-slate-900">{assignedSubjects.length}</div>
        ...
        <div className="bg-white rounded-[var(--radius)] border border-slate-200/90 p-5 shadow-xs hover:border-slate-300 transition-colors flex items-center justify-between">
          <div className="space-y-1">
            ...
            <div className="text-2xl font-medium text-slate-900">{totalModuleCount}</div>
        ...
        <div className="bg-white rounded-[var(--radius)] border border-slate-200/90 p-5 shadow-xs hover:border-slate-300 transition-colors flex items-center justify-between">
          <div className="space-y-1">
            ...
            <div className="text-2xl font-medium text-slate-900">{totalMaterials}</div>
            <span className="text-[11px] text-slate-500 font-medium">Videos, PDFs & Slides</span>
          </div>
          <div className="w-11 h-11 rounded-[var(--radius)] bg-slate-50 border border-slate-200 flex items-center justify-center text-slate-700">
      ...
      <div className="space-y-4">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-white p-4 rounded-[var(--radius)] border border-slate-200/90 shadow-xs">
          <div>
            <h3 className="text-xs font-semibold uppercase tracking-wider text-slate-700">Your Assigned Subject Curricula & Modules</h3>
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <select
              ...
              className="px-3 py-1.5 rounded-[var(--radius)] border border-slate-200 text-xs font-medium text-slate-700 bg-slate-50 focus:outline-none"
            >
            ...
            <div className="relative w-full sm:w-60">
              ...
              <input
                ...
                className="w-full pl-9 pr-3 py-1.5 rounded-[var(--radius)] border border-slate-200 text-xs focus:ring-1 focus:ring-[#0B3475] focus:outline-none"
              />
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {filteredSubjects.map((subject, idx) => (
            <div
              key={subject.id || idx}
              className="bg-white rounded-[var(--radius)] border border-slate-200/90 p-5 shadow-xs hover:border-slate-300 transition-colors flex flex-col justify-between space-y-3"
            >
              <div>
                <div className="flex items-center justify-between mb-2">
                  <span className="px-2 py-0.5 rounded text-[10px] font-medium bg-blue-50 text-[#0B3475] border border-blue-200">
                ...
                <h4 className="font-medium text-slate-900 text-sm mb-1 hover:text-[#0B3475] transition-colors">
          ...
          {filteredSubjects.length === 0 && (
            <div className="col-span-2 text-center py-12 bg-white rounded-[var(--radius)] border border-dashed border-slate-200 p-6 space-y-2">
              <div className="w-12 h-12 rounded-[var(--radius)] bg-blue-50 text-[#0B3475] flex items-center justify-center mx-auto">
