    ...
    <div className="fixed inset-0 z-50 bg-slate-900/70 backdrop-blur-xs flex items-center justify-center p-2 sm:p-4 overflow-y-auto animate-in fade-in font-sans">
      <div className="bg-white rounded-[var(--radius)] shadow-2xl border border-slate-200 w-full max-w-6xl max-h-[92vh] flex flex-col overflow-hidden text-slate-800">
        ...
        <div className="p-5 sm:p-6 bg-gradient-to-r from-slate-900 via-[#0a2558] to-indigo-950 text-white flex items-center justify-between shrink-0">
          ...
          <button
            onClick={onClose}
            className="p-2 hover:bg-white/10 rounded-[var(--radius)] text-white/70 hover:text-white transition-colors"
          >
            <X className="w-6 h-6" />
          </button>
        </div>
        ...
        <div className="bg-slate-50 p-4 border-b border-slate-200 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 text-xs">
          <div className="flex items-center gap-3 flex-wrap">
            ...
            <span className="px-3 py-1 bg-white rounded-[var(--radius)] border border-slate-200 font-semibold text-slate-800">
              Attempts &ge; <b>{minAttempts}</b>
            </span>
            <span className="text-slate-400 font-medium">+</span>
            <span className="px-3 py-1 bg-rose-50 border border-rose-200 text-rose-800 rounded-[var(--radius)] font-medium">
          ...
          <div className="flex items-center gap-2">
            <span className="font-black text-xs text-rose-700 bg-rose-100 px-3 py-1 rounded-[var(--radius)]">
          ...
          <div className="lg:col-span-5 p-4 sm:p-5 space-y-3 overflow-y-auto max-h-[calc(92vh-190px)] bg-slate-50/50">
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-9 pr-3 py-2 bg-white border border-slate-200 rounded-[var(--radius)] text-xs font-medium focus:outline-none focus:ring-2 focus:ring-indigo-600"
              />
            </div>
              ...
              {flaggedQuestions.map(q => {
                ...
                return (
                  <div
                    ...
                    className={`p-4 rounded-[var(--radius)] border transition-all cursor-pointer space-y-2.5 ${
                      isSelected
                  ...
                  >
                    <div className="flex items-start justify-between gap-2">
                      <div className="flex items-center gap-2">
                        <span className="px-2 py-0.5 rounded-[var(--radius)] bg-rose-100 text-rose-800 font-black text-xs">
                          Q{q.questionNumber}
                        </span>
                        <span className="text-xs font-medium text-slate-900">{q.topic}</span>
                      </div>
          ...
          <div className="lg:col-span-7 p-5 sm:p-6 space-y-5 overflow-y-auto max-h-[calc(92vh-190px)] bg-white">
            ...
            <div className="space-y-2 pb-4 border-b border-slate-100">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <span className="px-2.5 py-1 rounded-[var(--radius)] bg-rose-600 text-white font-black text-xs">
                ...
                <div className="flex items-center gap-2 text-xs font-black text-rose-700 bg-rose-50 px-3 py-1 rounded-[var(--radius)] border border-rose-200">
                  <TrendingDown className="w-4 h-4" />
                  <span>{selectedQuestion.accuracy}% Class Accuracy ({selectedQuestion.correct}/{selectedQuestion.attempts} Correct)</span>
                </div>
              </div>

              <div className="p-4 bg-slate-50 rounded-[var(--radius)] border border-slate-200 text-xs font-medium text-slate-900 leading-relaxed">
                  ...
                  <div key={idx} className="space-y-1">
                    <div className="flex items-center justify-between text-xs">
                      <div className="flex items-center gap-2 max-w-[80%]">
                        <span className={`w-5 h-5 rounded-full flex items-center justify-center font-medium text-[10px] ${
                          opt.isCorrect 
                        ...
                        <span className={`truncate ${opt.isCorrect ? "font-black text-emerald-950" : opt.isTrap ? "font-medium text-rose-950" : "text-slate-700"}`}>
                          {opt.text}
                        </span>
                      </div>
            ...
            <div className="space-y-3 pt-2">
              <div className="flex items-center gap-2 border-b border-slate-200 pb-2 overflow-x-auto text-xs font-medium">
                <button
                  ...
                  className={`px-3 py-1.5 rounded-[var(--radius)] transition-all flex items-center gap-1.5 shrink-0 ${
              ...
              {activeActionTab === "review" && (
                <div className="p-4 bg-slate-50 rounded-[var(--radius)] border border-slate-200 space-y-3 text-xs animate-in fade-in">
                  <div className="flex items-center justify-between">
                  ...
                  {isEditingMode ? (
                    <div className="space-y-3">
                      <div>
                        <label className="text-[10px] font-medium text-slate-400 uppercase">Question Text:</label>
                        <textarea
                          value={editingQuestionText}
                          onChange={(e) => setEditingQuestionText(e.target.value)}
                          rows={3}
                          className="w-full p-2.5 bg-white border border-slate-200 rounded-[var(--radius)] text-xs font-medium focus:ring-2 focus:ring-indigo-600"
                        />
                      </div>

                      <div>
                        <label className="text-[10px] font-medium text-slate-400 uppercase">Official Explanation & Working:</label>
                        <textarea
                      ...
                      <div className="flex justify-end gap-2 pt-1">
                        <button
                          ...
                          className="px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white font-black rounded-[var(--radius)] text-xs flex items-center gap-1.5 shadow-sm"
                        >
                    ...
                    <div className="space-y-2 text-slate-700">
                      ...
                      <div className="pt-2 flex items-center gap-2">
                        <button
                          ...
                          className="px-3.5 py-2 bg-blue-50 hover:bg-blue-100 text-blue-800 font-medium rounded-[var(--radius)] text-xs border border-blue-200 flex items-center gap-1.5"
                        >
              ...
              {activeActionTab === "ai_diagnostics" && (
                <div className="p-4 bg-indigo-50/60 rounded-[var(--radius)] border border-indigo-100 space-y-3 text-xs animate-in fade-in">
                  <div className="flex items-center gap-2">
                  ...
                  <div className="bg-amber-50 p-3.5 rounded-[var(--radius)] border border-amber-200 space-y-1 text-amber-950">
                    <p className="font-medium text-xs">Faculty Action Item:</p>
              ...
              {activeActionTab === "remedial_broadcast" && (
                <div className="p-4 bg-slate-50 rounded-[var(--radius)] border border-slate-200 space-y-3 text-xs animate-in fade-in">
                  ...
                  <textarea
                    ...
                    rows={6}
                    className="w-full p-3 bg-white border border-slate-200 rounded-[var(--radius)] text-xs font-mono focus:ring-2 focus:ring-blue-600"
                  />

                  <div className="flex justify-end gap-2">
                    <button
                      onClick={handleSendRemedialBroadcast}
                      className="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white font-black rounded-[var(--radius)] text-xs flex items-center gap-1.5 shadow-sm transition-transform hover:scale-105"
                    >
                      <Send className="w-3.5 h-3.5" />
              ...
              {activeActionTab === "generate_clones" && (
                <div className="p-4 bg-amber-50/60 rounded-[var(--radius)] border border-amber-200 space-y-3 text-xs animate-in fade-in">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                    ...
                    <button
                      ...
                      className="px-4 py-2 bg-amber-600 hover:bg-amber-700 disabled:opacity-50 text-white font-black rounded-[var(--radius)] text-xs flex items-center gap-1.5 shadow-sm transition-transform hover:scale-105"
                    >
                      {generatingClones ? (
                  ...
                  {clonedQuestions.length > 0 && (
                    <div className="space-y-2 pt-2">
                      {clonedQuestions.map((clone, idx) => (
                        <div key={idx} className="p-3.5 bg-white rounded-[var(--radius)] border border-amber-200 space-y-2 shadow-2xs">
                          <p className="font-medium text-slate-900">{clone.text}</p>
                          <div className="grid grid-cols-2 gap-1.5 text-[11px] text-slate-700">
                            {clone.options.map((opt, oIdx) => (
                              <div key={oIdx} className={`p-1.5 rounded-[var(--radius)] border ${oIdx === clone.correctAnswer ? "bg-emerald-50 border-emerald-300 font-medium text-emerald-900" : "bg-slate-50 border-slate-200"}`}>
        ...
        <div className="p-4 bg-slate-50 border-t border-slate-200 flex items-center justify-between shrink-0">
          ...
          <button
            onClick={onClose}
            className="px-5 py-2 bg-slate-200 hover:bg-slate-300 text-slate-800 font-semibold rounded-[var(--radius)] text-xs transition-colors"
          >
            Close Studio
          </button>
        </div>
