...
export const TrainerScheduleAssessmentView = ({ 
  ...
  const handleGenerateAiPaper = async (e) => {
    ...
    try {
      const promptTopic = `${createForm.subjectName} • ${aiPaperConfig.moduleName} • ${aiPaperConfig.topicName}: ${aiPaperConfig.conceptName}`;
      const res = await api.generateAiQuestions({
        topic: promptTopic,
        difficulty: aiPaperConfig.difficulty,
        count: Number(aiPaperConfig.questionCount),
        subjectName: createForm.subjectName,
        module: aiPaperConfig.moduleName
      });

      if (res.success && res.generatedQuestions?.length > 0) {
        setEditableAiPaper(res.generatedQuestions);
        showToast(`AI generated ${res.generatedQuestions.length} questions tailored for ${aiPaperConfig.moduleName}!`);
      } else {
            ...
            <div className="flex items-center gap-2">
              {!selectedQuizForDetails.resultsPublished ? (
                <button
                  ...
                  className="flex items-center gap-1.5 px-5 py-2.5 bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-700 text-white font-black rounded-xl text-xs shadow-md transition-transform hover:scale-105"
                >
