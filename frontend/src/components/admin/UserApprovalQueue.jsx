...
export const UserApprovalQueue = () => {
  ...
  const handleApprove = async (user, notes = "") => {
    ...
    try {
      const res = await api.verifyUser(user.id, true, notes || reviewNotes || "Profile authenticated by Central Administrator.");
      if (res.success) {
        alert(`✅ User ${user.name} verified and approved successfully!`);
        setInspectUser(null);
        setReviewNotes("");
        await fetchUsers();
      }
  ...
  const handleRejectConfirm = async () => {
    ...
    if (!rejectionReason.trim()) {
      alert("Please enter a specific reason or feedback for rejecting this user profile.");
      return;
    }
    ...
    try {
      ...
      if (res.success) {
        alert(`User ${rejectionModalUser.name} profile rejected. Rejection reason has been recorded and will be shown to the user.`);
  ...
  const rejectionPresets = [
    "Incomplete qualification certification documents. Please upload valid attested scan.",
    "User Registration ID mismatch with Central Personnel Registry. Please rectify details.",
    "Missing attested scientific/technical degree certificate in profile dossier.",
    "Departmental Head endorsement letter required.",
    "Uploaded credentials could not be verified with issuing institution."
  ];
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 space-y-6 max-w-7xl mx-auto font-sans text-slate-800 select-none">
      ...
      <div className="bg-white rounded-xl p-6 sm:p-8 text-slate-800 shadow-xs flex flex-col md:flex-row items-start md:items-center justify-between gap-6 border border-slate-200 relative overflow-hidden">
        <div className="space-y-1.5 max-w-2xl z-10">
          <div className="flex items-center gap-2">
            <span className="px-2.5 py-0.5 rounded-full text-[10px] font-semibold bg-blue-50 border border-blue-200 text-blue-800 uppercase tracking-wider">
              Verification & Approval Hub
            </span>
            <span className="text-xs text-slate-400 font-medium">Governance & Security</span>
          </div>
          <h1 className="text-xl sm:text-2xl font-bold tracking-tight text-slate-900">
            User Registration & Profile Approvals
          </h1>
          <p className="text-xs sm:text-sm text-slate-500 font-medium leading-relaxed">
            Verify academic credentials, certifications, and institutional postings before granting active capacity building access and course enrollment permissions.
          </p>
        </div>

        <button
          onClick={fetchUsers}
          className="flex items-center gap-2 px-4 py-2.5 bg-slate-50 hover:bg-slate-100 border border-slate-200 text-slate-700 rounded-lg text-xs font-medium transition-all shadow-xs active:scale-95 z-10"
        >
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        <div 
          ...
          className={`p-4 rounded-xl border transition-all cursor-pointer ${
            filterStatus === "pending" 
              ? "bg-blue-50/70 border-blue-300 ring-2 ring-blue-500 shadow-xs" 
        ...
        >
          <div className="flex items-center justify-between">
            <span className="text-xs font-medium text-slate-500">Verified Trainees</span>
            <GraduationCap className="w-4 h-4 text-blue-600" />
          </div>
          <p className="text-2xl font-bold text-blue-900 mt-2">{approvedTraineeCount}</p>
        ...
        >
          <div className="flex items-center justify-between">
            <span className="text-xs font-medium text-slate-500">Faculty Trainers</span>
            <Award className="w-4 h-4 text-indigo-600" />
          </div>
          <p className="text-2xl font-bold text-indigo-900 mt-2">{approvedTrainerCount}</p>
          <p className="text-[10px] text-indigo-700 mt-0.5 font-medium">Accredited Instructors</p>
        </div>
        ...
        <div 
          ...
          className={`p-4 rounded-xl border transition-all cursor-pointer ${
            filterStatus === "rejected" 
              ? "bg-rose-50/70 border-rose-300 ring-2 ring-rose-400 shadow-xs" 
              : "bg-white border-slate-200 hover:border-rose-200 hover:bg-slate-50/50 shadow-xs"
          }`}
        >
          <div className="flex items-center justify-between">
            <span className="text-xs font-medium text-slate-500">Rejected Profiles</span>
        ...
        <div className="flex flex-col sm:flex-row items-center gap-3">
          <div className="relative flex-1 w-full">
            ...
            <input
              ...
              placeholder="Search by User Name, Email, Registration ID, Station or Department..."
              className="w-full pl-10 pr-4 py-2.5 bg-slate-50 hover:bg-slate-100/80 focus:bg-white border border-slate-200 focus:border-blue-500 rounded-lg text-xs outline-none transition-all"
            />
            {searchQuery && (
              <button 
          ...
          {(filterStatus !== "all" || filterRole !== "all" || searchQuery) && (
            <button
              ...
              className="px-3 py-2 text-xs text-rose-600 hover:bg-rose-50 rounded-lg font-medium transition-colors whitespace-nowrap"
            >
              Reset Filters
            </button>
          )}
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-xs">
            <thead className="bg-slate-50 text-slate-600 font-semibold border-b border-slate-200">
              <tr>
                <th className="py-3.5 px-4 font-semibold">User Profile & ID</th>
                <th className="py-3.5 px-3 font-semibold">Role</th>
                <th className="py-3.5 px-3 font-semibold">Station & Division</th>
                <th className="py-3.5 px-3 font-semibold">Qualifications & Credentials</th>
              ...
              {loading ? (
                ...
                <tr>
                  <td colSpan="6" className="py-16 text-center text-slate-400">
                    ...
                    <p className="font-medium text-slate-700 text-sm">No officer records match the current filter.</p>
                      ...
                      <td className="py-4 px-4">
                        <div className="flex items-center gap-3">
                          <img
                            ...
                            alt={user.name}
                            className="w-10 h-10 rounded-[var(--radius)] object-cover ring-2 ring-slate-200 shrink-0"
                          />
                          <div>
                            ...
                            <span className="inline-block mt-0.5 px-2 py-0.2 rounded text-[10px] font-mono font-medium bg-slate-100 text-slate-700">
                        ...
                        <div className="flex items-center justify-end gap-1.5">
                          ...
                          {!isApproved && (
                            <button
                              ...
                              className="px-3 py-2 bg-emerald-600 hover:bg-emerald-700 text-white font-medium rounded-[var(--radius)] shadow-sm text-xs transition-transform hover:scale-105 flex items-center gap-1"
                          ...
                          {!isRejected && user.role !== "admin" && (
                            <button
                              ...
                              disabled={actionLoading}
                              className="px-2.5 py-2 bg-rose-50 hover:bg-rose-100 text-rose-700 font-medium rounded-[var(--radius)] text-xs transition-colors flex items-center gap-1"
      ...
      {/* ═════════ 1. COMPREHENSIVE USER DOSSIER MODAL ═════════ */}
      {inspectUser && (
          ...
          <div className="bg-white rounded-xl max-w-3xl w-full max-h-[90vh] overflow-y-auto shadow-xl border border-slate-200">
              ...
              <div className="relative z-10 flex items-start justify-between gap-4">
                <div className="flex items-center gap-4">
                  <img
                    ...
                    className="w-16 h-16 rounded-xl object-cover ring-4 ring-white/20 shadow-md"
                  />
                  <div>
                    <div className="flex items-center gap-2">
                      <h2 className="text-lg font-bold tracking-tight">{inspectUser.name}</h2>
                      <span className={`px-2.5 py-0.5 rounded-full text-[10px] font-semibold uppercase tracking-wider ${
                        inspectUser.role === "trainer" ? "bg-indigo-200 text-indigo-950" : "bg-blue-200 text-blue-950"
                      }`}>
                        {inspectUser.role}
                      </span>
                    </div>
                    ...
                    <div className="flex items-center gap-3 mt-2 text-[11px] text-blue-100 font-mono">
                      <span>Registration ID: <b>{inspectUser.cadreId || `ID-2026-${inspectUser.id.substring(inspectUser.id.length - 4)}`}</b></span>
                      <span>•</span>
                      <span>Station: <b>{inspectUser.station || "Central HQ"}</b></span>
                    </div>
                  </div>
                </div>
            ...
            <div className="p-6 space-y-6 text-xs text-slate-700">
              ...
              {inspectUser.status === "rejected" && (
                <div className="bg-rose-50 border border-rose-200 rounded-lg p-4 text-rose-900 space-y-1">
              ...
              {inspectUser.status === "pending" && (
                <div className="bg-amber-50 border border-amber-200 rounded-lg p-3.5 text-amber-900 flex items-center justify-between">
                  <div className="flex items-center gap-2 font-semibold text-amber-800">
                    <Clock className="w-4 h-4 text-amber-600" />
                    <span>Pending Verification Review — Action required by Admin</span>
                  </div>
                  <span className="text-[10px] font-semibold bg-amber-200/80 px-2 py-0.5 rounded-full">
                    Awaiting Decision
                  </span>
                </div>
              )}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 p-4 bg-slate-50 rounded-lg border border-slate-200/70">
                <div>
                  <span className="text-[10px] font-semibold text-slate-400 uppercase tracking-wider">Official Email</span>
                ...
                <div>
                  <span className="text-[10px] font-semibold text-slate-400 uppercase tracking-wider">Contact Phone</span>
                  <p className="font-semibold text-slate-800 truncate mt-0.5 flex items-center gap-1.5">
                ...
                <div>
                  <span className="text-[10px] font-semibold text-slate-400 uppercase tracking-wider">Operational Station</span>
                  <p className="font-semibold text-slate-800 truncate mt-0.5 flex items-center gap-1.5">
              ...
              <div className="p-4 rounded-lg bg-blue-50/60 border border-blue-200 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-lg bg-blue-600 text-white flex items-center justify-center shrink-0 shadow-xs">
                    <FileCheck2 className="w-5 h-5" />
                  </div>
                  <div>
                    <h4 className="font-semibold text-slate-900 text-xs">Official Registration & Personnel Service Record</h4>
                    <p className="text-[11px] text-slate-500">
                      Registration ID: {inspectUser.cadreId || "ID-2026-4491"} • Personnel Verification Database
                    </p>
                  </div>
                </div>
                <button
                  onClick={() => alert(`🔗 Verified Service Record: Document authenticated with Personnel Database for ${inspectUser.name}.`)}
                  className="px-3.5 py-2 bg-white hover:bg-blue-50 border border-blue-300 text-blue-700 font-semibold rounded-lg text-xs flex items-center gap-1.5 shadow-xs transition-transform hover:scale-[1.02] shrink-0"
                >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="p-4 rounded-lg border border-slate-200 bg-white space-y-2">
                  ...
                  <ul className="space-y-1.5 text-slate-600">
                    {toArray(inspectUser.qualifications).map((q, idx) => (
                      <li key={idx} className="p-2 bg-slate-50 rounded-md font-medium flex items-start gap-2">
                ...
                <div className="p-4 rounded-lg border border-slate-200 bg-white space-y-2">
                  <h4 className="font-bold text-slate-900 flex items-center gap-1.5">
                    ...
                    <span>Postings & Prior Experience</span>
                  </h4>
                  <ul className="space-y-1.5 text-slate-600">
              ...
              <div className="space-y-2">
                ...
                <div className="flex flex-wrap gap-1.5">
                  {(toArray(inspectUser.skills).length > 0 
                    ...
                    <span key={idx} className="px-3 py-1 bg-blue-50 text-blue-900 rounded-full text-xs font-medium border border-blue-200">
                      {skill}
                    </span>
                  ))}
                </div>
              ...
              <div className="space-y-3 pt-2">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  {(inspectUser.certificates || []).map((cert, cIdx) => (
                    <div key={cIdx} className="p-4 rounded-lg bg-slate-50 border border-slate-200 space-y-2 hover:border-blue-300 transition-colors">
                      <div className="flex items-start justify-between gap-2">
                        <h5 className="font-medium text-slate-900 text-xs leading-snug">{cert.title}</h5>
                        <span className="px-2 py-0.5 bg-emerald-100 text-emerald-800 rounded-full text-[9px] font-bold uppercase shrink-0">
                          {cert.grade || "Verified"}
                        </span>
                      </div>
                      <p className="text-[11px] text-slate-500">
                        Issuer: <b>{cert.issuer || "Training Division"}</b> • Year: <b>{cert.year || "2024"}</b>
                      </p>
                      <div className="pt-2 border-t border-slate-200/80 flex items-center justify-between">
                        <span className="text-[10px] font-mono text-slate-400">
                          ID: {cert.credentialId || `CERT-2024-${cIdx + 101}`}
                        </span>
                        <button
                  ...
                  {(inspectUser.certificates || []).length === 0 && (
                    <div className="col-span-2 p-6 text-center bg-slate-50 rounded-lg border border-dashed border-slate-300 text-slate-400">
                      <Award className="w-8 h-8 mx-auto mb-1 text-slate-300" />
                      <span>No certificates uploaded by this user yet.</span>
                    </div>
                  )}
                </div>
              </div>
              ...
              <div className="pt-2 space-y-1.5">
                ...
                <textarea
                  ...
                  placeholder="Optional verification concurrence note for central audit trail..."
                  className="w-full p-3 bg-slate-50 border border-slate-200 rounded-lg text-xs outline-none focus:border-blue-500"
                />
              </div>
            </div>
              ...
              <div className="flex items-center gap-2.5 w-full sm:w-auto">
                {inspectUser.role !== "admin" && (
                  <button
                    ...
                    disabled={actionLoading}
                    className="flex-1 sm:flex-none px-4 py-2.5 bg-rose-100 hover:bg-rose-200 text-rose-800 font-semibold rounded-lg text-xs transition-colors flex items-center justify-center gap-1.5"
                  >
                ...
                <button
                  ...
                  disabled={actionLoading}
                  className="flex-1 sm:flex-none px-6 py-2.5 bg-blue-600 hover:bg-blue-700 text-white font-semibold rounded-lg text-xs shadow-xs transition-all flex items-center justify-center gap-1.5"
                >
                  <UserCheck className="w-4 h-4 text-emerald-300" />
                  <span>Verify & Approve User Profile</span>
                </button>
              </div>
            </div>
          </div>
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-xs flex items-center justify-center p-4 z-50 animate-in fade-in">
          <div className="bg-white rounded-xl max-w-lg w-full p-6 shadow-xl border border-slate-200 space-y-4">
            ...
            <p className="text-xs text-slate-600 leading-relaxed">
              Type the exact reason for rejecting this user's profile. The <b>{rejectionModalUser.role}</b> will see this message prominently on their dashboard so they can rectify their credentials and resubmit.
            </p>
              ...
              <div className="flex flex-wrap gap-1.5">
                {rejectionPresets.map((preset, pIdx) => (
                  <button
                    ...
                    className="text-[10px] bg-slate-100 hover:bg-rose-50 text-slate-700 hover:text-rose-800 px-2.5 py-1 rounded-md border border-slate-200 transition-colors text-left"
                  >
            ...
            <div>
              ...
              <textarea
                ...
                placeholder="Explain what is missing or incorrect (e.g., Incomplete qualification certificate, registration ID mismatch, missing endorsement)..."
                className="w-full p-3 bg-slate-50 border border-slate-200 focus:border-rose-500 rounded-lg text-xs outline-none focus:ring-2 focus:ring-rose-200 transition-all"
              />
            </div>
            ...
            <div className="pt-2 flex items-center justify-end gap-2.5">
              ...
              <button
                ...
                className="px-5 py-2 bg-rose-600 hover:bg-rose-700 disabled:opacity-50 text-white font-semibold rounded-lg text-xs shadow-xs transition-all flex items-center gap-1.5"
              >
      ...
      {certificatePreviewModal && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-xs flex items-center justify-center p-4 z-50 animate-in fade-in">
          <div className="bg-white rounded-xl max-w-md w-full p-6 shadow-xl border border-slate-200 space-y-4 text-center">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500 mt-1">
                Issued by: <b>{certificatePreviewModal.issuer || "Training Academy"}</b> ({certificatePreviewModal.year || "2024"})
              </p>
            </div>

            <div className="p-3 bg-slate-50 rounded-lg border border-slate-200 text-xs font-mono text-slate-600 text-left space-y-1">
              <p>Credential ID: <b>{certificatePreviewModal.credentialId || "CERT-2024-8891"}</b></p>
              <p>Registry Status: <span className="text-emerald-700 font-semibold">ACTIVE & VALID</span></p>
              <p>Accreditation: <span className="text-blue-700">Standard Technical Competency</span></p>
            </div>

            <button
              onClick={() => setCertificatePreviewModal(null)}
              className="w-full py-2.5 bg-blue-600 hover:bg-blue-700 text-white font-semibold rounded-lg text-xs shadow-xs transition-colors"
            >
              Concur & Close
            </button>
          </div>
