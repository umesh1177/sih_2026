...
export const UserApprovalQueue = () => {
  ...
  const handleRejectConfirm = async () => {
    ...
    try {
      ...
      if (res.success) {
        alert(`X Officer ${rejectionModalUser.name} profile rejected. Rejection reason has been recorded and will be shown to the officer.`);
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 space-y-6 max-w-7xl mx-auto font-sans text-slate-800 select-none">
      ...
      <div className="bg-white rounded-[var(--radius)] p-6 sm:p-8 text-slate-800 shadow-sm flex flex-col md:flex-row items-start md:items-center justify-between gap-6 border border-slate-200/90 relative overflow-hidden">
        ...
        <button
          onClick={fetchUsers}
          className="flex items-center gap-2 px-5 py-2.5 bg-slate-50 hover:bg-slate-100 border border-slate-200 text-slate-800 rounded-[var(--radius)] text-xs font-semibold transition-all shadow-xs active:scale-95 z-10"
        >
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        <div 
          ...
          className={`p-4 rounded-[var(--radius)] border transition-all cursor-pointer ${
      ...
      <div className="bg-white rounded-[var(--radius)] p-4 sm:p-5 border border-slate-200 shadow-sm space-y-4">
        ...
        <div className="flex flex-wrap items-center gap-2 border-b border-slate-100 pb-3">
          ...
          {[
            ...
            <button
              ...
              className={`px-3.5 py-1.5 rounded-[var(--radius)] text-xs font-medium transition-all flex items-center gap-1.5 ${
                filterStatus === tab.id
        ...
        <div className="flex flex-col sm:flex-row items-center gap-3">
          <div className="relative flex-1 w-full">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2.5 bg-slate-50 hover:bg-slate-100/80 focus:bg-white border border-slate-200 focus:border-blue-500 rounded-[var(--radius)] text-xs outline-none transition-all"
            />
            {searchQuery && (
              <button 
          ...
          {(filterStatus !== "all" || filterRole !== "all" || searchQuery) && (
            <button
              ...
              className="px-3 py-2 text-xs text-rose-600 hover:bg-rose-50 rounded-[var(--radius)] font-medium transition-colors whitespace-nowrap"
            >
              Reset Filters
            </button>
          )}
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-xs">
            <thead className="bg-slate-50 text-slate-600 font-semibold border-b border-slate-200">
              <tr>
                <th className="py-4 px-4 font-medium">Officer Profile & Cadre ID</th>
                <th className="py-4 px-3 font-medium">Role</th>
                <th className="py-4 px-3 font-medium">Station & Division</th>
                <th className="py-4 px-3 font-medium">Qualifications & Credentials</th>
                <th className="py-4 px-3 font-medium text-center">Status</th>
                <th className="py-4 px-4 font-medium text-right">Verification Actions</th>
              </tr>
            </thead>
      ...
      {inspectUser && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 z-50 animate-in fade-in overflow-y-auto">
          <div className="bg-white rounded-[var(--radius)] max-w-3xl w-full max-h-[90vh] overflow-y-auto shadow-2xl border border-slate-200">
              ...
              <div className="relative z-10 flex items-start justify-between gap-4">
                <div className="flex items-center gap-4">
                  <img
                    ...
                    className="w-16 h-16 rounded-[var(--radius)] object-cover ring-4 ring-white/20 shadow-lg"
                  />
                  <div>
                    <div className="flex items-center gap-2">
            ...
            <div className="p-6 space-y-6 text-xs text-slate-700">
              ...
              {inspectUser.status === "rejected" && (
                <div className="bg-rose-50 border border-rose-200 rounded-[var(--radius)] p-4 text-rose-900 space-y-1">
              ...
              {inspectUser.status === "pending" && (
                <div className="bg-amber-50 border border-amber-200 rounded-[var(--radius)] p-3.5 text-amber-900 flex items-center justify-between">
                  <div className="flex items-center gap-2 font-medium text-amber-800">
                    <Clock className="w-4 h-4 text-amber-600" />
                    <span>Pending Verification Review — Action required by Admin</span>
                  </div>
                  <span className="text-[10px] font-medium bg-amber-200/80 px-2 py-0.5 rounded-full">
                    Awaiting Decision
                  </span>
                </div>
              )}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 p-4 bg-slate-50 rounded-[var(--radius)] border border-slate-100">
                <div>
                  <span className="text-[10px] font-medium text-slate-400 uppercase tracking-wider">Official Email</span>
                ...
                <div>
                  <span className="text-[10px] font-medium text-slate-400 uppercase tracking-wider">Contact Phone</span>
                  <p className="font-semibold text-slate-800 truncate mt-0.5 flex items-center gap-1.5">
                ...
                <div>
                  <span className="text-[10px] font-medium text-slate-400 uppercase tracking-wider">Operational Station</span>
                  <p className="font-semibold text-slate-800 truncate mt-0.5 flex items-center gap-1.5">
              ...
              <div className="p-4 rounded-[var(--radius)] bg-gradient-to-r from-blue-50 to-indigo-50 border border-blue-200 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-[var(--radius)] bg-blue-600 text-white flex items-center justify-center shrink-0 shadow-sm">
                    <FileCheck2 className="w-5 h-5" />
                  </div>
                  <div>
                    <h4 className="font-medium text-slate-900 text-xs">Official MoES Cadre ID & Service Record Document</h4>
                ...
                <button
                  ...
                  className="px-3.5 py-2 bg-white hover:bg-blue-50 border border-blue-300 text-blue-700 font-medium rounded-[var(--radius)] text-xs flex items-center gap-1.5 shadow-sm transition-transform hover:scale-105 shrink-0"
                >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {/* Qualifications */}
                <div className="p-4 rounded-[var(--radius)] border border-slate-200 bg-white space-y-2">
                  ...
                  <ul className="space-y-1.5 text-slate-600">
                    {toArray(inspectUser.qualifications).map((q, idx) => (
                      <li key={idx} className="p-2 bg-slate-50 rounded-[var(--radius)] font-medium flex items-start gap-2">
              ...
              <div className="space-y-3 pt-2">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  {(inspectUser.certificates || []).map((cert, cIdx) => (
                    <div key={cIdx} className="p-4 rounded-[var(--radius)] bg-slate-50 border border-slate-200 space-y-2 hover:border-blue-300 transition-colors">
                  ...
                  {(inspectUser.certificates || []).length === 0 && (
                    <div className="col-span-2 p-6 text-center bg-slate-50 rounded-[var(--radius)] border border-dashed border-slate-300 text-slate-400">
              ...
              <div className="pt-2 space-y-1.5">
                ...
                <textarea
                  ...
                  className="w-full p-3 bg-slate-50 border border-slate-200 rounded-[var(--radius)] text-xs outline-none focus:border-blue-500"
                />
              </div>
            </div>
            ...
            <div className="p-6 bg-slate-50 border-t border-slate-100 rounded-b-3xl flex flex-col sm:flex-row items-center justify-between gap-3">
              <button
                onClick={() => setInspectUser(null)}
                className="w-full sm:w-auto px-4 py-2.5 bg-slate-200 hover:bg-slate-300 text-slate-800 font-medium rounded-[var(--radius)] text-xs transition-colors"
              >
                Close Dossier
              </button>
              ...
              <div className="flex items-center gap-2.5 w-full sm:w-auto">
                {inspectUser.role !== "admin" && (
                  <button
                    ...
                    disabled={actionLoading}
                    className="flex-1 sm:flex-none px-4 py-2.5 bg-rose-100 hover:bg-rose-200 text-rose-800 font-medium rounded-[var(--radius)] text-xs transition-colors flex items-center justify-center gap-1.5"
                  >
                ...
                <button
                  ...
                  disabled={actionLoading}
                  className="flex-1 sm:flex-none px-6 py-2.5 bg-[#0a2558] hover:bg-[#071c42] text-white font-medium rounded-[var(--radius)] text-xs shadow-md transition-transform hover:scale-105 flex items-center justify-center gap-1.5"
                >
      ...
      {rejectionModalUser && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 z-50 animate-in fade-in">
          <div className="bg-white rounded-[var(--radius)] max-w-lg w-full p-6 shadow-2xl border border-slate-200 space-y-4">
            
            <div className="flex items-start justify-between pb-3 border-b border-slate-100">
              <div className="flex items-center gap-2.5">
                <div className="w-9 h-9 rounded-[var(--radius)] bg-rose-100 text-rose-700 flex items-center justify-center shrink-0">
            ...
            <div className="space-y-1.5">
              <span className="text-[10px] font-medium text-slate-400 uppercase tracking-wider">Quick Suggestions:</span>
              <div className="flex flex-wrap gap-1.5">
                {rejectionPresets.map((preset, pIdx) => (
                  <button
                    key={pIdx}
                    onClick={() => setRejectionReason(preset)}
                    className="text-[10px] bg-slate-100 hover:bg-rose-50 text-slate-700 hover:text-rose-800 px-2.5 py-1 rounded-[var(--radius)] border border-slate-200 transition-colors text-left"
                  >
            ...
            <div>
              <label className="block font-medium text-slate-800 text-xs mb-1">
              ...
              <textarea
                ...
                className="w-full p-3 bg-slate-50 border border-slate-200 focus:border-rose-500 rounded-[var(--radius)] text-xs outline-none focus:ring-2 focus:ring-rose-200 transition-all"
              />
            </div>
            ...
            <div className="pt-2 flex items-center justify-end gap-2.5">
              <button
                onClick={() => setRejectionModalUser(null)}
                className="px-4 py-2 bg-slate-100 hover:bg-slate-200 text-slate-700 font-medium rounded-[var(--radius)] text-xs transition-colors"
              >
                Cancel
              </button>
              <button
                ...
                className="px-5 py-2 bg-rose-600 hover:bg-rose-700 disabled:opacity-50 text-white font-semibold rounded-[var(--radius)] text-xs shadow-md transition-all flex items-center gap-1.5"
              >
      ...
      {certificatePreviewModal && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 z-50 animate-in fade-in">
          <div className="bg-white rounded-[var(--radius)] max-w-md w-full p-6 shadow-2xl border border-slate-200 space-y-4 text-center">
            <div className="w-12 h-12 rounded-[var(--radius)] bg-emerald-100 text-emerald-700 flex items-center justify-center mx-auto shadow-sm">
            ...
            <div className="p-3 bg-slate-50 rounded-[var(--radius)] border border-slate-200 text-xs font-mono text-slate-600 text-left space-y-1">
              <p>Credential ID: <b>{certificatePreviewModal.credentialId || "MOES-CERT-2024-8891"}</b></p>
              <p>Registry Status: <span className="text-emerald-700 font-medium">ACTIVE & VALID</span></p>
            ...
            <button
              ...
              className="w-full py-2.5 bg-[#0a2558] hover:bg-[#071c42] text-white font-medium rounded-[var(--radius)] text-xs shadow-md transition-colors"
            >
              Concur & Close
            </button>
          </div>
